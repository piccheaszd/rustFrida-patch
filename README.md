...
- 交互式 `--spawn`（不带 `-l`）现在先恢复子进程，等待主线程回到 Looper idle 后再按 PID attach。之前的普通 spawn 会在 zygote/setcontext 阻塞窗口里直接注入，目标早期线程很少，容易选到 ART 临时线程或撞上 UI 初始化，表现为 `jseval` 后 socket reset 或目标 native crash。
- `--spawn -l script.js` 仍保留恢复前注入和脚本加载，用于抢早期 hook；脚本加载完成后再恢复子进程，并等待应用进入可交互状态后进入 REPL。
