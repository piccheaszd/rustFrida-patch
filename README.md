...
- 已修复待复测：2026-06-29 BOCHK `pure spawn + minimal` baseline 可完成 agent 连接、`jsinit`、`jseval 1+1` 和 zygote patch restore，但 RF 退出后 BOCHK 仍留在前台进程，且 loader 注册的 `RF_ANTIDETECT=1` audit-only profile / hide-maps ranges 没有主动 release，只依赖目标进程 `exit_mmap` 清理。当前 CLI 在 `RF_ANTIDETECT` 开启时会在 shutdown 前发送 `kpm-release`，调用 `PR_ANTIDETECT_RELEASE` 和 `PR_HIDEMAPS_RELEASE(0,0)` 释放当前 `mm` 的 KPM 状态；测试时可加 `RF_FORCE_STOP_AFTER_EXIT=1`，让 RF 退出后 force-stop spawn 包，避免 BOCHK/安全组件残留继续拖住系统。这项需设备重启后重新做 BOCHK baseline 验证。
