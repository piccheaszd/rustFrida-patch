...
- 如果加载了 `anti-detect` 1.2.26 或更新版本，可通过环境变量让 loader 在进入 agent 前注册 per-mm profile。默认关闭；`RF_ANTIDETECT=1`、`RF_ANTIDETECT=audit` 或 `RF_ANTIDETECT=register-only` 只以 `AD_F_AUDIT_ONLY` 注册轻量 profile，不启用具体 syscall 检测面；需要全量审计时显式使用 `RF_ANTIDETECT=audit-full`，需要指定精确 flags 时设置 `RF_ANTIDETECT_FLAGS=0x...`，可选 `RF_ANTIDETECT_PROFILE_ID=<id>` 用于日志区分。KPM 侧默认 profile-only strict，未注册 App 不再走旧 UID/进程名 fallback；active exit/kill blocking 只接受 loader 预注册的 stage-1 / veneer 精确 range，不再按通用 caller path token 判断。启用 `RF_ANTIDETECT` 时，CLI 发送 `shutdown kpm-release`，由 agent 在目标进程退出清理路径内调用 `PR_ANTIDETECT_RELEASE` 和 `PR_HIDEMAPS_RELEASE(0,0)` 释放当前 `mm` 的 anti-detect profile 与 hide-maps ranges，避免 observe-only / baseline 会话退出后把 KPM 状态留到目标 App 自然退出；可用 `RF_KPM_RELEASE_ON_SHUTDOWN=0` 关闭，或在未启用 `RF_ANTIDETECT` 时用 `RF_KPM_RELEASE_ON_SHUTDOWN=1` 强制开启。
- BOCHK 这类会改写 `cmdline` / `status.Name` 的目标，identity watcher 不按具体伪装名字做 allow/deny list；只要 PID 来自本次 spawn/zygote 会话、UID 与目标一致，或属于目标 PID 的 child lineage，就记录为 `identity_alias`。因此无论 observed name 是 `com.google.android.gms`、`gapps`、Bluetooth 还是其它系统样式名字，都不会把 RF 目标改判为真实系统包。测试清理只使用原始 spawn 包、PID 和 UID：`RF_FORCE_STOP_AFTER_EXIT=1` 会先 `am force-stop <spawn package>`，再记录同 UID 残留；如果 target PID 已退出，会用 `cmd package list packages -U <spawn package>` 回退获取原始包 UID。默认只审计残留 PID，不发送信号；需要强清理时显式设置 `RF_KILL_SPAWN_UID_RESIDUALS=1`。`RF_FORCE_STOP_EXTRA_PACKAGES` 只用于手工指定确实要清的辅助包，RF 不会从 observed cmdline 自动追加包名。
...
- pre-resume 非 Java 脚本如果包含 `Hook.RECOMP` / `Java.RECOMP`，host 等待脚本加载结果时使用 15 秒超时；普通 stop-worker pre-resume 脚本仍使用 2 秒。BOCHK full-api RECOMP hook 安装实测会超过旧的 8 秒边界，过早恢复目标会造成脚本加载超时和 agent EOF。
...
- 已验证：2026-06-30 使用 boot-loaded `anti-detect` 1.2.37、`kpm-hide-maps` 1.1.2、`recompile`、`wxshadow`，BOCHK `pure spawn + minimal` baseline 在 `RF_ANTIDETECT=1 RF_FORCE_STOP_AFTER_EXIT=1 RF_SKIP_REPL_READY=1` 下可完成 agent 连接、`jsinit`、`jseval 1+1` 和 zygote patch restore；agent shutdown 时通过 `shutdown kpm-release` 释放当前 `mm` 的 audit-only anti-detect profile 和 hide-maps ranges，dmesg 可见 `anti-detect: released profile ... flags=0x1`，RF `rc=0` 且 adb 保持响应。多轮观察到 BOCHK 同 UID 进程把 `cmdline` / `status.Name` 改成 `com.google.process.gservices`、`com.android.nfc`、`com.google.android.inputmethod.latin`、`com.android.keych` 等系统样式名字，均按同 UID / 当前 spawn lineage 记录为 `identity_alias` 或 `SelfTraceChild`，清理只使用原始 `com.bochk.app.aos` 包名和 PID/UID。后续 `anti-detect` 1.2.38 已收紧 rule/profile 生命周期：纯 audit-only profile 不再接收 self-protect rule，persistent rule-only profile 仍保留给 PAIC stable 使用。
...
HTTP RPC 主要面向默认 ptrace 构建的 legacy/session 使用方式；`noptrace` 构建没有 server 和 process remote-call 后端。native-min agent 也不会编译 JS 侧 `rpc.exports` bootstrap，需要 RPC 时使用 `quickjs-full-api`。
